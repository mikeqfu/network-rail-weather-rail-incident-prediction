    ...
    **Example**::
    ...
    if os.path.isfile(path_to_pickle) and not update:
        ...
        try:
            ...
            rad_stations_info = pd.read_excel(path_to_spreadsheet,
                                              parse_dates=['Station start date'])
            rad_stations_info.columns = [x.title().replace(' ', '') if x != 'src_id' else x.upper()
                                         for x in rad_stations_info.columns]
            rad_stations_info.StationName = \
                rad_stations_info.StationName.str.replace(r'(\xa0)+Locate', '', regex=True)
...
def parse_midas_radtob(filename, headers, daily=False, rad_stn=False):
    ...
    :type headers: list
    :param daily: if True, 'OB_HOUR_COUNT' == 24, i.e. aggregate value in one day 24 hours;
        False (default)
    :type daily: bool
    ...
    raw_txt = pd.read_csv(filename, header=None, names=headers, parse_dates=[2, 12],
...
def make_midas_radtob_pickle_path(data_filename, daily, rad_stn):
    ...
    path_to_radtob_pickle = cdd_weather(
        "MIDAS", "{}{}.pickle".format(data_filename, *filename_suffix))

    return path_to_radtob_pickle


def get_midas_radtob(data_filename="midas-radtob-2006-2019", daily=False, update=False,
                     verbose=False):
    """
    ...
    if os.path.isfile(path_to_pickle) and not update:
        ...
        try:
            ...
            headers_raw = pd.read_excel(
                cdd_weather("midas", "radiation-observation-data-headers.xlsx"), header=None)
            ...
            with zipfile.ZipFile(path_to_zip, 'r') as zf:
                ...
                temp_dat = [parse_midas_radtob(zf.open(f), headers, daily, rad_stn=False)
...
def dump_midas_radtob_to_mssql(table_name='MIDAS_RADTOB', if_exists='append', chunk_size=100000,
    ...
    **Example**::
    ...
    for chunk in temp_file_:
        ...
        chunk.to_sql(table_name, midas_radtob_engine, schema='dbo', if_exists=if_exists,
                     index=False, dtype=dtype_, method='multi')
        gc.collect()
...
def dump_midas_supplement_to_mssql(table_name='MIDAS_RADTOB_suppl', if_exists='replace',
                                   update=False, verbose=False):
    """
    ...
    **Example**::
    ...
    if supplement_data is not None and not supplement_data.empty:
        midas_radtob_engine = create_mssql_connectable_engine(database_name='Weather')
        supplement_data.to_sql(table_name, midas_radtob_engine, schema='dbo', if_exists=if_exists,
                               index=False, dtype={'OB_END_DATE': sqlalchemy.types.DATE})


def query_midas_radtob_by_grid_datetime(met_stn_id, period, route_name, use_suppl_dat=False,
                                        update=False, dat_dir=None, pickle_it=False,
                                        verbose=False):
    """
    ...
    **Examples**::
        ...
        midas_radtob = query_midas_radtob_by_grid_datetime(met_stn_id, period, pickle_it=pickle_it,
                                                           verbose=verbose)
    """
    ...
    if os.path.isfile(path_to_pickle) and not update:
        ...
        ms_id = met_stn_id[0]
        dates = tuple(x.strftime('%Y-%m-%d %H:%M:%S')
                      for x in [period.left.min(), period.right.max()])
        sql_query = "SELECT * FROM dbo.[MIDAS_RADTOB] " \
                    "WHERE [SRC_ID] = {} " \
                    "AND [OB_END_DATE_TIME] BETWEEN '{}' AND '{}';".format(ms_id,
                                                                           dates[0], dates[1])
        ...
        if midas_radtob.empty and use_suppl_dat:
            ...
            sql_query = "SELECT * FROM dbo.[MIDAS_RADTOB_suppl] " \
                        "WHERE [Route] = '{}' " \
                        "AND [OB_END_DATE] BETWEEN '{}' AND '{}';".format(route_name,
                                                                          dates[0], dates[1])
            midas_radtob = pd.read_sql(sql_query, conn_metex)
