...
def cleanse_location_metadata_tiploc_sheet(metadata, update_dict=False):
    ...
    dat.replace(fetch_location_names_repl_dict('LOOKUP_NAME'), inplace=True)
    dat.replace(fetch_location_names_repl_dict('LOOKUP_NAME', regex=True), inplace=True)

    # Fill in missing location names
    na_name = dat[dat.LOOKUP_NAME.isnull()]
    lid = LocationIdentifiers()
    ref_dict = lid.make_location_codes_dictionary(ref_cols, update=update_dict)
    ...
    na_name = dat[dat.LOOKUP_NAME.isnull()]
    ref_dict = lid.make_location_codes_dictionary('TIPLOC', update=update_dict)
    ...
    stn = Stations()
    station_data = stn.fetch_railway_station_data()['Railway station data']
...
def get_location_metadata(update=False, verbose=False):
    """
    ...
    **Example**::
    ...
    if os.path.isfile(path_to_pickle) and not update:
        ...
        try:
            ...
            print("Failed to fetch \"STANOX locations data\" from {}. {}".format(os.path.dirname(path_to_pickle), e))
    ...
    **Example**::
    ...
    if os.path.isfile(path_to_pickle) and not update:
        ...
        try:
            ...
            metex_location.WeatherCell = metex_location.WeatherCell.map(lambda x: '' if pd.isna(x) else str(int(x)))
    ...
    **Example**::
    ...
    if os.path.isfile(path_to_pickle) and not update:
        ...
        try:
            ...
            metex_stanox_location.loc[na_desc.index, 'Description':'Name'] = temp[['Location', 'STANME']].values
            ...
            rc_tiploc_dict = lid.make_location_codes_dictionary('TIPLOC')
            ...
            metex_stanox_location.loc[temp.index, 'Description':'Name'] = temp[['Location', 'STANME']].values
            ...
            loc_name_replacement_dict = fetch_location_names_repl_dict('Description')
            metex_stanox_location.replace(loc_name_replacement_dict, inplace=True)
            loc_name_regexp_replacement_dict = fetch_location_names_repl_dict('Description', regex=True)
            ...
            rc_stanox_dict = lid.make_location_codes_dictionary('STANOX')
            ...
            if not valid_loc.empty:
                metex_stanox_location.loc[valid_loc.index, 'Description'] = valid_loc.apply(
                    lambda x: find_similar_str(x[1], x[2]) if isinstance(x[2], list) else x[2], axis=1)

            # Check if 'Description' has TIPLOC code instead of location name using STANOX-TIPLOC-dictionary
            rc_stanox_tiploc_dict = lid.make_location_codes_dictionary(['STANOX', 'TIPLOC'])
            ...
            rc_stanox_stanme_dict = lid.make_location_codes_dictionary(['STANOX', 'STANME'])
            ...
            metex_stanox_location.Yards = metex_stanox_location.Yards.map(lambda x: '' if pd.isna(x) else str(int(x)))

            # Clear duplicates by 'STANOX'
            temp = metex_stanox_location[metex_stanox_location.duplicated(subset=['Stanox'], keep=False)]
...
def get_metex_stanox_section(update=False, verbose=False):
    """
    ...
    **Example**::
    ...
    if os.path.isfile(path_to_pickle) and not update:
        ...
        try:
            ...
            metex_stanox_section.rename(columns={'Id': 'StanoxSectionId', 'Description': 'StanoxSection'}, inplace=True)
            ...
            #
            stanox_location = get_metex_stanox_location(update)
            ...
            if not valid_loc.empty:
                stanox_sec.loc[valid_loc.index, 'Start'] = valid_loc.apply(
                    lambda x: find_similar_str(x[0], x[1]) if isinstance(x[1], (list, tuple)) else x[1], axis=1)
            ...
            # Apply manually-created dictionaries
            loc_name_replacement_dict = fetch_location_names_repl_dict('Start')
            stanox_sec.replace(loc_name_replacement_dict, inplace=True)
            loc_name_regexp_replacement_dict = fetch_location_names_repl_dict('Start', regex=True)
            stanox_sec.replace(loc_name_regexp_replacement_dict, inplace=True)
            loc_name_replacement_dict = fetch_location_names_repl_dict('End')
            stanox_sec.replace(loc_name_replacement_dict, inplace=True)
            loc_name_regexp_replacement_dict = fetch_location_names_repl_dict('End', regex=True)
            stanox_sec.replace(loc_name_regexp_replacement_dict, inplace=True)
            ...
            metex_stanox_section[raw_col_names] = stanox_sec[['StanoxSection', 'Start_Raw', 'End_Raw']]
...
def get_location_metadata_plus(update=False, verbose=False):
    """
    ...
    **Example**::
    ...
    if os.path.isfile(path_to_pickle) and not update:
        ...
        try:
            # Location
            location = get_metex_location(update=update, verbose=verbose)

            # # STANOX location
            stanox_location = get_metex_stanox_location(update=update, verbose=verbose)
            ...
            # STANOX section
            stanox_section = get_metex_stanox_section(update=update, verbose=verbose)
            ...
            # --------------------------------------------------------------------------
            ref = get_location_metadata(update=update, verbose=verbose)['STANOX']
            ...
            loc_meta_plus.loc[temp.index, 'StartLongitude':'StartLatitude'] = temp[['DEG_LONG', 'DEG_LAT']].values
            loc_meta_plus.loc[temp.index, 'ApproximateStartLocation'] = False
            loc_meta_plus.loc[loc_meta_plus.index.difference(temp.index), 'ApproximateStartLocation'] = True

            temp = loc_meta_plus.join(ref, on=['EndStanox', 'EndLocation'])
            temp = temp[temp.DEG_LAT.notnull() & temp.DEG_LONG.notnull()]
            loc_meta_plus.loc[temp.index, 'EndLongitude':'EndLatitude'] = temp[['DEG_LONG', 'DEG_LAT']].values
            loc_meta_plus.loc[temp.index, 'ApproximateEndLocation'] = False
            loc_meta_plus.loc[loc_meta_plus.index.difference(temp.index), 'ApproximateEndLocation'] = True
...
def cleanse_stanox_section_column(data, col_name='StanoxSection', sep=' : ', update_dict=False):
    ...
    lid = LocationIdentifiers()
    #
    stanox_dict = lid.make_location_codes_dictionary('STANOX', update=update_dict)
    start_end = tidy_alt_codes(stanox_dict, start_end)
    #
    stanme_dict = lid.make_location_codes_dictionary('STANME', update=update_dict)
    start_end = tidy_alt_codes(stanme_dict, start_end)
    #
    tiploc_dict = lid.make_location_codes_dictionary('TIPLOC', update=update_dict)
...
def cleanse_geographical_coordinates(data, update_metadata=False):
    ...
    # ref 2 ----------------------------------------------
    stn = Stations()
    ref_metadata_2 = stn.fetch_railway_station_data()['Railway station data']
...
def get_schedule8_weather_incidents(route_name=None, weather_category=None, update=False, verbose=False):
    """
    ...
    **Example**::
    ...
    if os.path.isfile(path_to_pickle) and not update:
        ...
        try:
            ...
            s8weather_incidents.rename(columns={'StartDate': 'StartDateTime',
                                                'EndDate': 'EndDateTime',
            ...
            s8weather_incidents = s8weather_incidents.join(incident_reason_metadata,
                                                           on='IncidentReason', rsuffix='_meta')
            s8weather_incidents.drop([x for x in s8weather_incidents.columns if '_meta' in x], axis=1, inplace=True)

            # Cleanse the location data
            s8weather_incidents = cleanse_stanox_section_column(s8weather_incidents, col_name='StanoxSection',
                                                                sep=' : ', update_dict=update)
...
def get_schedule8_weather_incidents_02062006_31032014(route_name=None, weather_category=None, update=False,
    ...
    **Example**::
        ...
        s8weather_incidents = get_schedule8_weather_incidents_02062006_31032014(route_name, weather_category,
                                                                                update, verbose)
        print(s8weather_incidents)
    """
    ...
    if os.path.isfile(path_to_pickle) and not update:
        ...
        try:
            ...
            # 'Data' ------------------------------------------------------------------------------------------------
            incident_records = workbook.parse(sheet_name='Data', parse_dates=['StartDate', 'EndDate'], dayfirst=True,
                                              converters={'stanoxSection': str})
            incident_records.rename(columns={'StartDate': 'StartDateTime',
                                             ...
                                             # 'Cost': 'DelayCost',
                                             'CategoryDescription': 'IncidentCategoryDescription'}, inplace=True)
            hazard_cols = [x for x in enumerate(incident_records.columns) if 'Weather Hazard' in x[1]]
            ...
            incident_reason_metadata = get_incident_reason_metadata()
            incident_reason_metadata.columns = [c.replace('_', '') for c in incident_reason_metadata.columns]
            incident_records = incident_records.join(incident_reason_metadata, on='IncidentReason', rsuffix='_meta')
            incident_records.drop([x for x in incident_records.columns if '_meta' in x], axis=1, inplace=True)

            # Cleanse the location data
            incident_records = cleanse_stanox_section_column(incident_records, col_name='StanoxSection', sep=' : ',
                                                             update_dict=update)
            ...
            # Make a dictionary
            s8weather_incidents = dict(zip(workbook.sheet_names,
                                           [thresholds, incident_records, weather_category_lookup]))
            ...
            print('Failed to get/parse \"Schedule8WeatherIncidents-02062006-31032014.xlsm\". {}.'.format(e))
            s8weather_incidents = None
