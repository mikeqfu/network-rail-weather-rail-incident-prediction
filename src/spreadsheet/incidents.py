...
from pyhelpers import find_similar_str, osgb36_to_wgs84, wgs84_to_osgb36, load_json, load_pickle, \
    save_pickle
    ...
    **Example**::
    ...
    if os.path.isfile(path_to_pickle) and not update:
        ...
        try:
            ...
            tiploc_loc_lyr = cleanse_location_metadata_tiploc_sheet(tiploc_loc_lyr,
                                                                    update_dict=update)
    ...
    **Example**::
    ...
    if os.path.isfile(path_to_pickle) and not update:
        ...
        try:
            ...
            incident_reason_metadata.columns = [c.replace('_', '') for c in
            ...
            s8weather_incidents.drop([x for x in s8weather_incidents.columns if '_meta' in x],
                                     axis=1, inplace=True)

            # Cleanse the location data
            s8weather_incidents = cleanse_stanox_section_column(s8weather_incidents,
                                                                col_name='StanoxSection',
            ...
            s8weather_incidents = get_subset(s8weather_incidents, route_name, weather_category,
...
def get_schedule8_weather_incidents_02062006_31032014(route_name=None, weather_category=None,
                                                      update=False,
                                                      verbose=False):
    """
    ...
    **Example**::
    ...
    # Path to the file
    pickle_filename = make_filename("Schedule8WeatherIncidents-02062006-31032014", route_name,
                                    weather_category)
    path_to_pickle = cdd_incidents("Spreadsheets", pickle_filename)
    ...
    if os.path.isfile(path_to_pickle) and not update:
        ...
        try:
            ...
            incident_records.rename(columns={'StartDate': 'StartDateTime',
                                             ...
                                             'CategoryDescription': 'IncidentCategoryDescription'},
                                    inplace=True)
            hazard_cols = [x for x in enumerate(incident_records.columns) if
                           'Weather Hazard' in x[1]]
            obs_cols = [(i - 1, re.search(r'(?<= \()\w+', x).group().upper()) for i, x in
                        hazard_cols]
            ...
            incident_reason_metadata.columns = [c.replace('_', '') for c in
                                                incident_reason_metadata.columns]
            incident_records = incident_records.join(incident_reason_metadata, on='IncidentReason',
                                                     rsuffix='_meta')
            incident_records.drop([x for x in incident_records.columns if '_meta' in x], axis=1,
                                  inplace=True)

            # Cleanse the location data
            incident_records = cleanse_stanox_section_column(incident_records,
                                                             col_name='StanoxSection', sep=' : ',
                                                             update_dict=update)
            ...
            # Retain data for specific Route and Weather category
            incident_records = get_subset(incident_records, route_name, weather_category,
                                          rearrange_index=False)
