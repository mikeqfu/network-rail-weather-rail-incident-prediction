...
def prepare_shp_layer_files(osm_subregion,
...
def get_shp_file_path_for_basemap(osm_subregion, osm_layer, osm_feature=None,
    ...
    **Examples**::
    ...
    try:
        ...
        path_to_shp_file = \
        dri.find_osm_shp_file(osm_subregion, osm_layer, osm_feature, shp_file_dir)[0]
    ...
    if boundary_polygon is not None:
        ...
        if not os.path.isfile(path_to_sub_shp_file) or update:
            try:
                subarea_data = dri.read_shp(path_to_shp_file, mode='geopandas',
            ...
            subarea_data.crs = {'no_defs': True, 'ellps': 'WGS84', 'datum': 'WGS84',
                                'proj': 'longlat'}
            subarea_data.to_file(path_to_sub_shp_file, driver='ESRI Shapefile',
                                 encoding='UTF-8')
...
def plot_base_map(projection='tmerc', railway_line_color='#3D3D3D',
    ...
    **Example**::
    ...
    # base_map.arcgisimage(service='World_Shaded_Relief', xpixels=1500, dpi=300,
    #                      verbose=False)
    ...
    boundary_polygon = shapely.geometry.Polygon(zip(base_map.boundarylons,
                                                    base_map.boundarylats))
    path_to_shp_file = get_shp_file_path_for_basemap('England', 'railways', 'rail',
                                                     boundary_polygon, 'anglia')
    ...
    font = matplotlib.font_manager.FontProperties(family='Cambria', weight='normal',
                                                  size=16)
    legend = plt.legend(numpoints=1, loc='best', prop=font, frameon=False, fancybox=True,
...
def plot_weather_cells(base_map=None, update=False, weather_cell_colour='#D5EAFF',
                       legend_loc=(1.05, 0.85)):
    """
    ...
    **Example**::
    ...
    for i in range(len(data)):
        ...
        p = matplotlib.patches.Polygon(list(xy), fc=weather_cell_colour, ec='#4b4747',
                                       zorder=2)
        plt.gca().add_patch(p)

    # Add labels
    # plt.plot([], 's', label="Weather cell", ms=30, color=weather_cell_colour, markeredgecolor='#433f3f', alpha=.5)
    plt.plot([], 's', label="Weather cell", ms=25, color='#D5EAFF',
             markeredgecolor='#433f3f', alpha=.5)

    # Show legend  # font = {'family': 'Georgia', 'size': 16, 'weight': 'bold'}
    font = matplotlib.font_manager.FontProperties(family='Cambria', weight='normal',
                                                  size=16)
    plt.legend(numpoints=1, loc='best', prop=font, frameon=False, fancybox=True,
               bbox_to_anchor=legend_loc)

    print("Done.")


def plot_osm_forest_and_tree(base_map=None, osm_landuse_forest_colour='#72886e',
    ...
    **Example**::
    ...
    if fill_forest_patches:
        ...
        forest_polygons = [
            matplotlib.patches.Polygon(p, fc=osm_landuse_forest_colour,
                                       ec=osm_landuse_forest_colour, zorder=4)
    ...
    if add_osm_natural_tree:
        ...
        base_map.scatter([geom.x for geom in natural_tree_points],
                         [geom.y for geom in natural_tree_points],
    ...
    plt.legend(scatterpoints=10, loc='best', prop=font, frameon=False, fancybox=True,
               bbox_to_anchor=legend_loc)
...
def plot_hazardous_trees(base_map=None, hazardous_tree_colour='#ab790a', legend_loc=(1.05, 0.85)):
    ...
    **Example**::
    ...
    base_map.scatter([geom.x for geom in hazardous_trees_points],
                     [geom.y for geom in hazardous_trees_points],
...
def plot_base_map_plus(show_metex_weather_cells=True, show_osm_landuse_forest=True,
                       add_osm_natural_tree=False,
                       show_nr_hazardous_trees=True, legend_loc=(1.05, 0.85), save_as=None,
    ...
    **Example**::
    ...
    if show_osm_landuse_forest:
        plot_osm_forest_and_tree(base_map, add_osm_natural_tree=add_osm_natural_tree,
    ...
    sr = fig.add_axes([0.58, 0.01, 0.40, 0.40],
                      frameon=True)  # quantities are in fractions of figure width and height
    sr.imshow(PIL.Image.open(
        cdd_network("Routes\\Map", "NR-Routes-edited-1.tif")))  # "Routes-edited-0.png"
    sr.axis('off')
...
def get_midpoints_for_plotting_hotspots(route_name=None, weather_category=None, sort_by=None,
                                        update=False,
                                        verbose=False):
    """
    ...
    **Example**::
    ...
    if os.path.isfile(path_to_pickle) and not update:
        ...
        if route_name:
            path_to_boundary_polygon = cdd_network(
                "Routes\\{}\\boundary-polygon.pickle".format(route_name))
            boundary_polygon = load_pickle(path_to_boundary_polygon)
            sub_area_name = route_name
        else:
            boundary_polygon, sub_area_name = None, None

        railway_coordinates = get_shp_coordinates('England', 'railways', 'rail', boundary_polygon,
                                                  sub_area_name)
        ...
        midpoints_ = pd.DataFrame(midpoints, schedule8_costs_by_location.index,
    ...
    incident_hotspots = get_subset(incident_hotspots, route_name=route_name,
...
def get_schedule8_annual_stats(route_name='Anglia', weather_category='Wind', update=False,
    ...
    **Example**::
    ...
    if os.path.isfile(path_to_pickle) and not update:
        ...
        schedule8_data = metex.view_schedule8_costs_by_datetime_location(route_name,
                                                                         weather_category, update)
        ...
        incident_location_midpoints = get_midpoints_for_plotting_hotspots(route_name,
                                                                          weather_category,
                                                                          update=update)
        ...
        annual_stats.sort_values(by=['DelayMinutes', 'DelayCost', 'IncidentCount'], ascending=False,
                                 inplace=True)
...
def plot_hotspots_given_annual_stats(route_name='Anglia', weather_category='Wind',
                                     update_data=False, cmap_name='Set1',
    ...
    **Example**::
    ...
    d_label = ["%s  (%s min." % (fy, format(int(d), ",")) for fy, d in
               zip(f_years, hotspots_annual_stats.DelayMinutes)]
    ...
    for y, fy in zip(years, f_years):
        ...
        for i in plot_data.index:
            ...
            base_map.plot(mid_x, mid_y, zorder=2, marker='o', color=colours[years.index(y)],
                          alpha=0.9,
...
def plot_hotspots_for_delays(route_name='Anglia', weather_category='Wind', update_data=False,
    ...
    **Example**::
    ...
    hotspots_data_init = get_midpoints_for_plotting_hotspots(route_name, weather_category,
                                                             sort_by=['DelayMinutes',
                                                                      'IncidentCount', 'DelayCost'],
                                                             update=update_data)
...
def plot_hotspots_for_incident_frequency(route_name='Anglia', weather_category='Wind',
                                         update_data=False, seed=1,
    ...
    **Example**::
    ...
    hotspots_data_init = get_midpoints_for_plotting_hotspots(route_name, weather_category,
                                                             sort_by=['IncidentCount', 'DelayCost',
                                                                      'DelayMinutes'],
                                                             update=update_data)
...
def plot_hotspots_for_delays_per_incident(route_name='Anglia', weather_category='Wind',
                                          update_data=False,
...
def plot_hotspots_on_route(route_name='Anglia', weather_category='Wind', update_data=False):
    ...
    **Example**::
    ...
    if confirmed():  # No need of dpi for ".pdf" or ".svg"
        ...
        # Annual delays     # Fig. 2.
        plot_hotspots_given_annual_stats(route_name, weather_category, update_data, 'Set1', True,
                                         True, True, fmt)
        plot_hotspots_given_annual_stats(route_name, weather_category, update_data, 'Set1', True,
                                         True, True, ".tif",
                                         dpi=600)
        ...
        # Delays    # Fig. 3.
        plot_hotspots_for_delays(route_name, weather_category, update_data, 1, 'Reds', False, False,
                                 False, fmt)
        plot_hotspots_for_delays(route_name, weather_category, update_data, 1, 'Reds', True, True,
                                 True, ".tif",
                                 dpi=600)

        # Cost
        plot_hotspots_for_cost(route_name, weather_category, update_data, 1, 'YlGnBu', False, False,
                               False, fmt)
        plot_hotspots_for_cost(route_name, weather_category, update_data, 1, 'YlGnBu', True, True,
                               True, fmt)
