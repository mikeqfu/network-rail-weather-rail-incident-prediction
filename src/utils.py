...
def get_data_by_season(incident_data, season='summer'):
    ...
    if season:
        ...
        for y in pd.unique(incident_data.FinancialYear):
            ...
            spring_dates1 = pd.date_range(
                spring_start1, spring_start1 + pd.DateOffset(months=2))
            spring_dates2 = pd.date_range(
                spring_start2, spring_start2 + pd.DateOffset(months=2))
            spring_data = pd.concat(
                [spring_data, y_dat[y_dat.StartDateTime.isin(spring_dates1) |
                                    y_dat.StartDateTime.isin(spring_dates2)]])

            spring_time = ((y_dat.StartDateTime >= spring_start1) &
                           (y_dat.StartDateTime < spring_end1)) | \
                          ((y_dat.StartDateTime >= spring_start2) &
                           (y_dat.StartDateTime < spring_end2))
            ...
            summer = (y_dat.StartDateTime >= summer_start) & \
                     (y_dat.StartDateTime < summer_end)
            ...
            autumn = (y_dat.StartDateTime >= autumn_start) & \
                     (y_dat.StartDateTime < autumn_end)
            ...
            winter = (y_dat.StartDateTime >= winter_start) & \
                     (y_dat.StartDateTime < winter_end)
        ...
        seasonal_data = eval(
            "pd.concat([%s], ignore_index=True)" % ', '.join(
                ['{}_data'.format(s) for s in season]))
...
def make_filename(base_name, route_name=None, weather_category=None, *extra_suffixes, sep="-",
                  save_as=".pickle"):
    """
        ...
        if route_name:
            try:  # assert 'Route' in data_subset.columns
                ...
                route_name_ = [find_similar_str(x, route_lookup)
                               for x in ([route_name] if isinstance(route_name, str)
                                         else list(route_name))]
