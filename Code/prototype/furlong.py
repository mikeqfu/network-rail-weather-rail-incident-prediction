...
def fetch_nr_vegetation_furlong_data(update=False) -> pd.DataFrame:
    ...
    path_to_pickle = prototype.tools.cd_prototype_dat(pickle_filename + ".pickle")
...
def get_connecting_nodes(route_name=None, update=False) -> pd.DataFrame:
    ...
    path_to_pickle = prototype.tools.cd_prototype_dat(pickle_filename)
    if os.path.isfile(path_to_pickle) and not update:
        return load_pickle(path_to_pickle)
    else:
        try:
            path_to_pickle_temp = prototype.tools.cd_prototype_dat(mssqlserver.metex.make_filename(filename))
